...
const SCRIPT = [
  ...
  {
    who: "bot",
    text: "Bonsoir 🌙 Oui, on livre jusqu'à 23h ! Vous souhaitez voir le menu du soir ?",
    t: "19:42",
  },
  ...
  {
    who: "bot",
    text: "Parfait ✅  7 500 F + 1 000 F livraison. Lien Wave envoyé !",
    t: "19:44",
  },
];
...
function Bubble({ msg, index }) {
  ...
  return (
    <div
      ...
      style={{
        animation: `msgSlideUp 0.45s cubic-bezier(0.34,1.56,0.64,1) both`,
        animationDelay: "0ms",
      }}
    >
      {!isUser && <BotAvatar />}
      ...
      <div
        className={`max-w-[72%] ${isUser ? "items-end" : "items-start"} flex flex-col gap-1`}
      >
        <div
          className={`px-3.5 py-2.5 rounded-2xl text-[13px] leading-[1.5] ${
            isUser
        ...
        <span className="text-[10px] text-neutral-400 dark:text-neutral-600 px-1">
          {msg.t}
        </span>
      </div>
    </div>
...
function TypingDots() {
  return (
    <div
      ...
      style={{ animation: "msgSlideUp 0.35s ease-out both" }}
    >
      <BotAvatar />
      ...
      <div className="w-[340px] rounded-3xl overflow-hidden shadow-[0_32px_64px_-16px_rgba(0,0,0,0.18),0_8px_32px_-8px_rgba(0,0,0,0.08)] border border-neutral-200/80 dark:border-neutral-700/50 bg-white/95 dark:bg-neutral-900/95 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-neutral-100 dark:border-neutral-800 bg-white dark:bg-neutral-900">
          <div className="relative">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary-500 to-primary-700 grid place-items-center text-white font-bold text-sm shadow-sm">
              K
            </div>
